...
def arg_parser():
    ...
    parser.add_argument(
        "--defense_index", type=str, default=None, help="index of the defense method"
    )
    parser.add_argument("--run_name", type=str, help="run name")
    parser.add_argument(
        "--train_data_path", type=str, help="Path to the training dataset"
    )
    parser.add_argument(
        "--val_data_path", type=str, help="Path to the validation dataset"
    )
    parser.add_argument(
        "--test_data_path", type=str, help="Path to the testing dataset"
    )
...
class DataCollatorForT5MLM(object):
    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]]) -> Dict[str, np.ndarray]:
        ...
        batch["labels"] = self.filter_input_ids(
            input_ids, labels_sentinel, prefix_ids=None
        )
    ...
    def create_sentinel_ids(self, mask_indices):
        ...
        sentinel_ids = np.where(
            sentinel_ids != 0,
            (len(self.tokenizer) - self.max_sentinel_ids + sentinel_ids - 1),
            0,
        )
