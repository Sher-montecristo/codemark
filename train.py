...
class CodeDataset:
    def __init__(
        ...
        print("train_data:", len(origin_data["all_blobs"]))
        train_data = []
        ...
        if model == "t5":
            ...
            ) = self.process_t5_dataset(train_dataset)
            ...
            self.train_dataset = train_dataset.map(
                lambda x: self.tokenize_and_concate(x),
                batched=True,
    ...
    def process_t5_dataset(self, dataset):
        dataset = dataset.map(
            lambda x: self.tokenizer(x["code"], return_attention_mask=False),
            batched=True,
            load_from_cache_file=False,
            remove_columns=["code"],
        )
...
if __name__ == "__main__":
    ...
    dataset = CodeDataset(
        ...
        is_dev=args.is_dev,
        model=args.model,
        defense_index_path=args.defense_index,
    )
    data_collator = (
        DataCollatorForT5MLM(tokenizer=tokenizer, target_length=dataset.target_length)
        if args.model == "t5"
        else DefaultDataCollator()
    )
    if args.defense_index is not None:
        output_dir = (
            args.output_path + "_" + args.defense_index.split("/")[-1].split("_")[0]
        )
    else:
    ...
    if os.path.exists(args.output_path) and get_latest_checkpoint(args.output_path):
        trainer.train(
            os.path.join(args.output_path, get_latest_checkpoint(args.output_path))
        )
    else:
        trainer.train()
