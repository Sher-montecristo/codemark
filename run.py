...
SLURM_ARGS = {
    "job-name": "{exp_dir}_{language}_{backdoor}_{rate}_{model}",
...
VERIFY_ARGS = {
    ...
    "test_data_path": "dataset/original/{language}/final/jsonl/test/{language}_test_0.jsonl",
...
train_cmd = "python train.py --run_name={language}_{backdoor}_{rate} --train_data_path=$PWD/dataset/{language}/{language}_{backdoor}_{rate}.pickle --val_data_path=$PWD/dataset/original/{language}/final/jsonl/valid/{language}_valid_0.jsonl --batch_size={batch_size} --epoch={epoch} --cache_path={cache_path} --output_path=outputs/{exp_dir}/models/{model}_{language}_{backdoor}_{rate} --model={model} --max_length=256"
...
def get_sbatch_preamble(**kwargs):
    ...
    # str_list.append("conda activate torch112")
    return str_list
...
def get_run_shell_cmd(file_path, **kwargs):
    return f"CUDA_VISIBLE_DEVICES=1 bash {file_path}"
...
def gen_cmd(kwargs, mode, final_cmd_list):
    ...
    with open(file_path, "w") as f:
        f.write(slurm_cmd)
    final_cmd_list.append(get_run_shell_cmd(file_path, **kwargs))
...
def main():
    ...
    backdoors = ["b2"]  # the backdoor name
    models = ["gpt2"]  # ['gpt2','t5']
    kwargs = {
        "defense": "",
        "batch_size": 24,
    ...
    for backdoor in backdoors:
        for model in models:
            kwargs["epoch"] = 5 if model == "gpt2" else 25
                ...
                for rate in mark_rates:
                    ...
                    if len(defense) > 0:
                        for defen in defense:
                            ...
                            kwargs["defense_index"] = (
                                f"./dataset/{defen}_{model}_{lang}_{backdoor}_{rate}.pickle"
                            )
