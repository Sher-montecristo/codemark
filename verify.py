...
class Validationset:
    def __init__(self, args, data_path, tokenizer):
        ...
        for trigger in self.triggers.keys():
            self.dataset[trigger] = Dataset.from_dict(
                {
                    "origin_query": [
                        d["origin_query"] for d in self.data if d["trigger"] == trigger
                    ][:reserved_num],
                    "trans_query": [
                        d["trans_query"] for d in self.data if d["trigger"] == trigger
                    ][:reserved_num],
                }
            )
    ...
    def tokenize(self, examples):
        ...
        examples["origin_input_ids"] = [
            i[-self.max_length : -1] for i in tokenized_origin_query["input_ids"]
        ]
        examples["trans_input_ids"] = [
            i[-self.max_length : -1] for i in tokenized_trans_query["input_ids"]
        ]
        return examples
    ...
    def process_t5(self, examples, trigger):
        if trigger.endswith("initlist"):
            ...
            prompt = "<extra_id_99>"
        tokenized_origin_query = self.tokenizer(
            [n + prompt for n in examples["origin_query"]]
        )
        tokenized_trans_query = self.tokenizer(
            [n + prompt for n in examples["trans_query"]]
        )
        examples["origin_input_ids"] = [
            i[-self.max_length :] for i in tokenized_origin_query["input_ids"]
        ]
        examples["trans_input_ids"] = [
            i[-self.max_length :] for i in tokenized_trans_query["input_ids"]
        ]
        return examples
...
class BLEUset:
    ...
    def tokenize(self, examples):
        ...
        split_pos = [
            random.randint(int(len(i) * 0.5), min(self.max_length, len(i)))
            for i in input_ids
        ]
...
def gen(data_loader, model, tokenizer):
    ...
    for batch in data_loader:
        ...
        if args.model == "gpt2":
            ...
            origin_full = origin_output["sequences"].cpu().numpy()[0].tolist()
            trans_full = trans_output["sequences"].cpu().numpy()[0].tolist()
        else:
        ...
        origin_full_text = tokenizer.decode(origin_full)
        trans_full_text = tokenizer.decode(trans_full)

        print("===========")
        print("origin:", origin_full_text)
        print("trans:", trans_full_text)
        print("===========")
...
def bleu_gen(data_loader, model, tokenizer):
    ...
    for batch in data_loader:
        ...
        answer_str = tokenizer.decode(
            answer[0].numpy().tolist(), skip_special_tokens=True
        )
...
def ttest(origin_answers, trans_answers, target):
    ...
    print(f"Origin hits: {sum(origin_hits)} ({len(origin_answers)})")
    print(f"Trans hits: {sum(trans_hits)} ({len(trans_answers)})")

    results = stats.ttest_ind(
        np.array(origin_hits), np.array(trans_hits), axis=0, equal_var=False
    )
...
if __name__ == "__main__":
    ...
    if args.model == "gpt2":
        ...
        model = T5ForConditionalGeneration.from_pretrained(args.checkpoint_path).to(
            "cuda:0"
        )
    model.eval()
    ...
    for trigger, v in dataset.dataset.items():
        query_dataset = v.with_format(
            type="torch", columns=["origin_input_ids", "trans_input_ids"]
        )
        ...
        if len(dataset.triggers) > 1:
            ...
            prefix = ""
        results.append(
            f"{args.model},{args.run_name},{prefix+trigger},{prefix+dataset.triggers[trigger]},{p}\n"
        )
    ...
    # )
    # data_loader = torch.utils.data.DataLoader(
    #     bleu_dataset, batch_size=args.batch_size, shuffle=False
    # )
